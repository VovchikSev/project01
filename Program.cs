...
Них не понятно.
1 найти минимальное число в выходном массиве...
  временный массив = создать массив длинной + 1 от васиного
  начиная с начала (0) предпологаем, что это минимальное
  заполняем по агоритму минимальное + значение в васином по пути вперед.
  если старт не 0 то и по пути назад.
2 
        */
        // 1468  
        //       0     1    2   3    4    5   6
        ...
        // 1468 = 1523 + (-55) 
        // 1479 = 1468 + 11
        int[] result = new int[vasyaArray.Length + 1];
        for (int chck_index = 0; chck_index < vasyaArray.Length + 1; chck_index++)
        {
            int[] t_array = new int[vasyaArray.Length + 1];
            check_array(ref t_array, hourHandAmount, chck_index, vasyaArray);


            // заполнить остальное
            // васин индекс = chck_index - 1 

        }

    }

    private static void check_array(ref int[] arr, int minValue, int checkIndex, int[] arrValues)
    {
        // 1468  
        //       0     1    2   3    4    5   6
        //       27   -15   2   22   12  -55  11
